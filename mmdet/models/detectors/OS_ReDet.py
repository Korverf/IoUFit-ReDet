...
class OS_ReDet(BaseDetectorNew, RPNTestMixin):
    ...
    def init_weights(self, pretrained=None):
        super(OS_ReDet, self).init_weights(pretrained)
    ...
    def forward_train(self,
        ...
        if self.with_bbox:
            ...
            loss_bbox = self.bbox_head.loss(cls_score, bbox_pred, rois, *rbbox_targets)
        ...
        if self.with_rbbox:
            ...
            rbbox_feats_cls, rbbox_feats_reg = self.rbbox_roi_extractor(x[:self.rbbox_roi_extractor.num_inputs], rrois)
            if self.with_shared_head_rbbox:
                rbbox_feats_cls = self.shared_head_rbbox(rbbox_feats_cls)
                rbbox_feats_reg = self.shared_head_rbbox(rbbox_feats_reg)
            cls_score, rbbox_pred = self.rbbox_head(rbbox_feats_cls, rbbox_feats_reg)
            rbbox_targets = self.rbbox_head.get_target_rbbox(sampling_results, gt_obbs, gt_labels,
                                                             self.train_cfg.rcnn[1])
            loss_rbbox = self.rbbox_head.loss(cls_score, rbbox_pred, rrois, *rbbox_targets)
    ...
    def simple_test(self, img, img_meta, proposals=None, rescale=False):
        ...
        rbbox_feats_cls, rbbox_feats_reg = self.rbbox_roi_extractor(x[:len(self.rbbox_roi_extractor.featmap_strides)], rrois_enlarge)

        if self.with_shared_head_rbbox:
            rbbox_feats_cls = self.shared_head_rbbox(rbbox_feats_cls)
            rbbox_feats_reg = self.shared_head_rbbox(rbbox_feats_reg)

        rcls_score, rbbox_pred = self.rbbox_head(rbbox_feats_cls, rbbox_feats_reg)
