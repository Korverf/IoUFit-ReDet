...
from .. import os_riroi_align_cuda


class OSRiRoIAlignFunction(Function):
    ...
    def forward(ctx, features, rois, out_size, spatial_scale, sample_num=0, nOrientation=8):
        ...
        if features.is_cuda:
            os_riroi_align_cuda.forward(features, rois, out_h, out_w, spatial_scale,
    ...
    def backward(ctx, grad_output_cls, grad_output_reg):
        ...
        if ctx.needs_input_grad[0]:
            grad_input = rois.new_zeros(batch_size, num_channels, data_height,
                                        data_width)
            os_riroi_align_cuda.backward(grad_output_cls.contiguous(), grad_output_reg.contiguous(), rois, out_h,
...
os_riroi_align = OSRiRoIAlignFunction.apply
