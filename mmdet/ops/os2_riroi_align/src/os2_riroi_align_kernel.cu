...
template <typename scalar_t>
__global__ void OS2RiROIAlignForward(const int nthreads, const scalar_t *bottom_data,
...
template <typename scalar_t>
__global__ void OS2RiROIAlignBackward(
...
int OS2RiROIAlignBackwardLaucher(const at::Tensor top_grad_cls, const at::Tensor top_grad_reg, const at::Tensor rois,
    ...
    at::Tensor bottom_grad) {
        const int output_size = num_rois * pooled_height * pooled_width * channels * nOrientation;
        AT_DISPATCH_FLOATING_TYPES_AND_HALF(
            top_grad_cls.type(), "OS2RiROIAlignLaucherBackward", ([&] {
              ...
              OS2RiROIAlignBackward<scalar_t>
                  <<<GET_BLOCKS(output_size), THREADS_PER_BLOCK>>>(
