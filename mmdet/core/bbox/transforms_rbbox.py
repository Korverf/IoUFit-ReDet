...
def best_match_dbbox2delta(Rrois, gt, means = [0, 0, 0, 0, 0], stds=[1, 1, 1, 1, 1]):
    ...
    return bbox_targets, gt_boxes_new
...
def obb2hbb(dbboxes):
    """
    get enclosed boxes
    :param dbboxes: shape (n, 5) (x_ctr, y_ctr, w, h, angle)
    :return: enclosed_boxes: shape (n, 4) (xmin, ymin, xmax, ymax)
    """
    polys = RotBox2Polys_torch(dbboxes) # n, 8
    xmin, _ = torch.min(polys[:, 0::2], dim=1)
    xmax, _ = torch.max(polys[:, 0::2], dim=1)
    ymin, _ = torch.min(polys[:, 1::2], dim=1)  # N
    ymax, _ = torch.max(polys[:, 1::2], dim=1)

    enclosed_boxes = torch.cat([xmin.unsqueeze(-1), ymin.unsqueeze(-1), xmax.unsqueeze(-1), ymax.unsqueeze(-1)], 1) # n, 4
...
def rroi2roi(rrois):
    """
    :param rrois: Tensor: shape (n, 6), [batch_ind, x, y, w, h, theta]
    :return: rois: Tensor: shape (n, 5), [batch_ind, x1, y1, x2, y2]
    """
    obbs = rrois[:, 1:]
    hbbs = obb2hbb(obbs)

    return torch.cat((rrois[:, 0].unsqueeze(1), hbbs), 1)

# def rroi2mask(rrois, masks)
#     """
#     :param rrois, mask: empty mask
#     :return: mask
#     """
#     obbs = rrois[:, 1:]
#     masks = obb2mask(obbs)
