...
def bbox_target_rbbox(pos_bboxes_list,
    ...
    # import pdb
    # pdb.set_trace()
    labels, label_weights, bbox_targets, bbox_weights, bbox_targets_decode = multi_apply(
    ...
    if concat:
        ...
        bbox_targets_decode = torch.cat(bbox_targets_decode, 0)
        bbox_weights = torch.cat(bbox_weights, 0)
    return labels, label_weights, bbox_targets, bbox_weights, bbox_targets_decode
...
def bbox_target_rbbox_single(pos_bboxes,
    ...
    bbox_targets_decode = pos_bboxes.new_zeros(num_samples, 5)
    ...
    if num_pos > 0:
        ...
        bbox_targets_decode[:num_pos, :] = pos_gt_obbs
...
def rbbox_target_rbbox_single(pos_rbboxes,
    ...
    bbox_targets_decode = pos_rbboxes.new_zeros(num_samples, 5)
    # TODO optimizer it
    # import pdb
    # pdb.set_trace()
    ...
    if num_pos > 0:
        ...
        # import pdb
        # pdb.set_trace()
        pos_bbox_targets, pos_bbox_targets_decode = best_match_dbbox2delta(pos_rbboxes, pos_gt_rbboxes, target_means,
                                      target_stds)
        bbox_targets[:num_pos, :] = pos_bbox_targets
